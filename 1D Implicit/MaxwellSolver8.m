function [Bh,E1h,E2h] = MaxwellSolver8(Lev,Deg,Hash,InvHash,Con1D,GradX,...
                                 omega,dt,MaxT,...
...
%     time=time+dt;
    time = T*dt;
    bbb=-[F_1D.b3*(time^2-time);F_1D.b1*(pi^2*time^2+time);F_1D.b2*time];
    b0=-[F_1D.b3*((time-dt)^2-(time-dt));F_1D.b1*(pi^2*(time-dt)^2+time-dt);F_1D.b2*(time-dt)];
