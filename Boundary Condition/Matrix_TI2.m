function [GradX,GradY] = Matrix_TI2(Lev,Deg,Lmax,FMWT_COMP_x)
%========================================================
...
for Lx=0:nx-1
    ...
    elseif Lx==0
        ...
        GradY=GradY+sparse(1:Deg,1,val3,dof_1D_x,1);
        
    elseif Lx==nx-1
        
        Iu=[meshgrid(p),meshgrid(c),meshgrid(c),meshgrid(c)];
        GradX=GradX+sparse(Iv,Iu,val2,dof_1D_x,dof_1D_x);
        GradY=GradY+sparse(Deg*(nx-1)+1:Deg*(nx),1,val4,dof_1D_x,1);
    end
