function [Eh,Bh] = MaxwellSolver(Lev,Deg,Hash,InvHash,Con1D,GradX,GradY,...
...
ComMat4 = sparse(Dofs1,Dofs1);
ComMat5 = sparse(Dofs1,Dofs1);
ComMat6 = sparse(Dofs1,Dofs1);
...
for i = 1:Dof_Hash
    ...
    Mat4=sparse(Dofs1,Dofs1);
    Mat5=sparse(Dofs1,Dofs1);
    Mat6=sparse(Dofs1,Dofs1);
    %****************************
    ...
    for j = 1:size(jIndex1D,2)
        ...
        if jLev1+jLev2+jLev3<=Lev
            ...
            tmp_mat4 = GradY(Deg*(iInd2-1)+1:Deg*iInd2,Deg*(jIndex1D(j)-1)+1:Deg*jIndex1D(j));
            tmp_mat1 = eye(Deg);tmp_mat3=eye(Deg);
            tmp_mat = kron(kron(tmp_mat1,tmp_mat2),tmp_mat3);
            tmp_mat11= kron(kron(tmp_mat1,tmp_mat4),tmp_mat3);
            ...
            Mat5=Mat5+sparse(II,JJ,tmp_mat11,Dofs1,Dofs1);
    ...
    for j = 1:size(jIndex1D,2)
        ...
        if jLev1+jLev2+jLev3<=Lev
            ...
            tmp_mat4 = GradY(Deg*(iInd1-1)+1:Deg*iInd1,Deg*(jIndex1D(j)-1)+1:Deg*jIndex1D(j));
            tmp_mat23 = speye(Deg^2);
            tmp_mat = kron(tmp_mat1,tmp_mat23); %"-"sign
            tmp_mat11=kron(tmp_mat4,tmp_mat23);
            ...
            Mat6=Mat6+sparse(II,JJ,tmp_mat11,Dofs1,Dofs1);
        end
    end
        ComMat1=ComMat1+Mat1;
        ComMat2=ComMat2+Mat2;
        ComMat3=ComMat3+Mat3;
        ComMat4=ComMat4+Mat4;
        ComMat5=ComMat5+Mat5;
        ComMat6=ComMat6+Mat6;
...
MaxMat=[zero,zero,zero,zero,ComMat1/(eps*mu),-ComMat2/(eps*mu);
        ...
        zero,-ComMat4,ComMat5,zero,zero,zero;
        ComMat4,zero,-ComMat6,zero,zero,zero;
        -ComMat5,ComMat6,zero,zero,zero,zero;];
% opts.tol=1e-20;
% spy(MaxMat)
