function [ perm, iperm, pvec ] = sg_to_fg_unknown_mapping( unknown )
...
assert( num_dims <= 2, 'sg_to_fg_unknown_mapping only implemented for num_dims<=2' )

deg = unknown.deg;
A_data = unknown.A_data;

if( num_dims == 1 )

    num_sparse_ele = numel(A_data.element_local_index_D{1}); %Number of sparse elements

    perm  = (1:num_sparse_ele*deg)';
    iperm = perm;
    pvec  = perm;
    ...
    max_x_dof = deg * 2^(max(unknown.hash_table.elements.lev_p1(:,1))-1);
    max_y_dof = deg * 2^(max(unknown.hash_table.elements.lev_p1(:,2))-1);
