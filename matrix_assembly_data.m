function [ A_Data ] = matrix_assembly_data( input_unknown, output_unknown, opts )
...
if opts.use_oldhash
    N = size(output_unknown.hash_table,2);
else
    Ne = numel(output_unknown.hash_table.elements_idx);
    N = Ne;
end
nDims = numel(output_unknown.dimensions);
...
for workItem = 1:N
    ...
    if opts.use_oldhash
        thisRowBasisCoords = output_unknown.hash_table{workItem};
    else
    end
    ...
    for d=1:nDims
        ...
        if opts.use_oldhash
            ...
            IDlev = output_unknown.hash_table.elements.lev_p1(output_unknown.hash_table.elements_idx(workItem),d)-1;
            IDpos = output_unknown.hash_table.elements.pos_p1(output_unknown.hash_table.elements_idx(workItem),d)-1;
