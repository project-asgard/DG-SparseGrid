...
run_time_advance(fokkerplanck1_pitch_E(1),out_format, ...
    'CFL',0.01,'lev',2,'deg',2,'grid_type','SG','timestep_method','RK3');
...
levels = [2, 3];
run_time_advance(fokkerplanck2_complete,out_format, ...
    'CFL',0.01,'lev',levels,'deg',3,'grid_type','SG',...
    'timestep_method', 'BE');
...
function run_time_advance(pde, out_format, varargin)
  ...
  if ~(length(unique(opts.lev_vec)) == 1)
      ...
      for i=1:length(pde.dimensions)
         lev_string = lev_string + int2str(opts.lev_vec(i)) + "_";
      end
  ...
  for i=0:4
      time = i*dt;
      out = time_advance(pde,opts,A_data,out,time,dt,opts.deg,hash_table,Vmax,Emax);
