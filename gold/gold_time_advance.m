...
% 
% %sg l2d2
% out_format = strcat(batch_dir, 'fokkerplanck1_4p2_sg_l%i_d%i_t%d.dat');
...
% 
% %sg l2d2
% out_format = strcat(batch_dir, 'fokkerplanck1_4p2_fg_l%i_d%i_t%d.dat');
% pde = fokkerplanck1_4p2;
% level = 2;
% degree = 2;
% grid_type='FG';
...
%     out_format = strcat(batch_dir, 'fokkerplanck1_4p1a_sg_l%i_d%i_t%d.dat');
...
for i=1:5
    [err,fval,fval_realspace] = asgard (fokkerplanck1_4p3, ...
        'lev',lev, 'deg',deg, 'CFL', 0.01, 'use_oldhash', true, 'num_steps', i, 'quiet', true);
    out_format = strcat(batch_dir, 'fokkerplanck1_4p3_sg_l%i_d%i_t%d.dat');
    write_octave_like_output(sprintf(out_format,lev,deg,i-1), full(fval));
end
