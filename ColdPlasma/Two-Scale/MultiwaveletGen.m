...
%     for j_s = 1:k
%         proj = zeros(1,k);
%         phi_x_j_s = sum(polyval(phi_co(j_p,:),(p_legendre_roots-1)/2).*polyval(scale_co(j_s,:),(p_legendre_roots-1)/2).*p_legendre_weights)/2+...
%             sum(polyval(phi_co(j_p+k,:),(p_legendre_roots+1)/2).*polyval(scale_co(j_s,:),(p_legendre_roots+1)/2).*p_legendre_weights)/2;
%         if abs(phi_x_j_s)>1e-13
%             phi_x_j_s
%         end
%     end
% end
...
% G0 = G0(k:-1:1,:);
% save(['two_scale_rel_',num2str(k),'.mat'],'H0','G0','scale_co','phi_co','-mat');
