...
% mu=1.2566370614e-6;
    ...
    function f=rhs2(x,y,z,t)
        f(:,1)=-eps*cos(2*pi*x).*sin(2*pi*y).*sin(2*pi*z).*sin(w*t)*w+12*pi^2*cos(2*pi*x).*sin(2*pi*y).*sin(2*pi*z).*sin(w*t)/w/mu;
        f(:,2)= x-x;
        f(:,3)= eps*sin(2*pi*x).*sin(2*pi*y).*cos(2*pi*z).*sin(w*t)*w-12*pi^2*sin(2*pi*x).*sin(2*pi*y).*cos(2*pi*z).*sin(w*t)/w/mu;
    end
