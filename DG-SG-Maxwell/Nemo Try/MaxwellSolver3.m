...
for i = 1:Dof_Hash
   ...
   jIndex2 = Con1D{iInd2};
   jIndex3 = Con1D{iInd3};
   size1 = size(jIndex1,2);
   size2 = size(jIndex2,2);
   size3 = size(jIndex3,2);
   J = [];
   Mat=sparse(Dofs1,Dofs1);
    
   I=[1:size3]';J=[1:size2]';K=[1:size1]';
   II=repmat(I,size2,1);
   JJ=repmat(J,1,size3)';
   KK=repmat(K,1,size2*size3)';
   index=[JJ(:),II];
   index=repmat(index,size1,1);
   J=[KK(:),index];
   
               
               
         for k= 1:size1*size2*size3
             j1=J(k,1);j2=J(k,2);j3=J(k,3);
           ...
           if jLev1+jLev2+jLev3<=Lev
                  ...
                  JJ = Deg^3*(tmp-1)+1:Deg^3*tmp;
                  [II,JJ] = meshgrid(JJ,II);
                  add=sparse(II,JJ,up,Dofs1,Dofs1);
                  Mat=Mat+add;            %Why it is so slow
           end
...
MaxMat=[Mat/(eps*mu),zero,zero,zero,ComMat1/(eps*mu),-ComMat2/(eps*mu);
        zero,Mat/(eps*mu),zero,-ComMat1/(eps*mu),zero,ComMat3/(eps*mu);
        zero,zero,Mat/(eps*mu),ComMat2/(eps*mu),-ComMat3/(eps*mu),zero;
        zero,-ComMat1,ComMat2,Mat,zero,zero;
        ComMat1,zero,-ComMat3,zero,Mat,zero;
        -ComMat2,ComMat3,zero,zero,zero,Mat;];
