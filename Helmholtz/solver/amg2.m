function [x,info,Ai,Bi,BBi,Res,Pro,cl] = amg2(A,b,option,varargin)
...
x0 = option.x0; N0 = option.N0; tol = option.tol; maxIt = option.solvermaxit;
...
switch preconditioner
    case 'V'
        prefunc = @vcycle;
        if printlevel >= 1
            fprintf('Algebraic Multigrid V-cycle Preconditioner with ')
        end
    case 'W'
        prefunc = @wcycle;
        if printlevel >= 1
            fprintf('Algebraic Multigrid W-cycle Preconditioner with ')
        end
    case 'F'
        prefunc = @fcycle;
        if printlevel >= 1
            fprintf('Algebraic Multigrid Full Cycle Preconditioner with ')
        end
    case 'BPX'
        % modify smoother
        for j = level:-1:2
...
switch solver
    ...
    case 'VCYCLE'
        if printlevel >= 1
            fprintf('Algebraic Multigrid Vcycle Iteration \n')
        end
        while (max(err(k,:)) > tol) && (k <= maxIt)
            k = k + 1;
    ...
    case 'WCYCLE'
        if printlevel >= 1
            fprintf('Algebraic Multigrid Wcycle Iteration \n')
        end
        while (max(err(k,:)) > tol) && (k <= maxIt)
            k = k + 1;
            % Step 2: Compute Br by one Vcylce MG
    ...
    case 'FCYCLE'
        if printlevel >= 1
            fprintf('Algebraic Multigrid Full Cycle Iteration \n')
        end
        while (max(err(k,:)) > tol) && (k <= maxIt)
            k = k + 1;
