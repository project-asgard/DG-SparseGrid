% Term 1
% x*d/dp(p^2*E*f)
Mat_Term1_p = MatrixGrad(Lev,Deg,pInt,pEnd,0,@(x)x.^2.*E,@(x)0,fp_bcL,fp_bcR);
...
Mat_Term2_p = MatrixMass(Lev,Deg,pInt,pEnd,@(x)(-x)); 
Mat_Term2_x = MatrixGrad(Lev,Deg,LInt,LEnd,0,@(x)(1-x.^2)*E,@(x)0,fx_bcL,fx_bcR);
