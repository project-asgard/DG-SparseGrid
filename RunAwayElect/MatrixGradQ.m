function Mat = MatrixGradQ(Lev,Deg,LInt,LEnd,FluxVal,FunCoef,FunCoef2,bcL,bcR)
...
%             ];
        TraVal = [...
            (-p_L)' * (p_L-p_L),...(-p_L)' * FunCoef(xL) * p_R/2 + (-1) * abs(FunCoef(xL))/2 * (-p_L)' *   p_R,...
            (-p_L)' * FunCoef(xL) * p_L,...(-p_L)' * FunCoef(xL) * p_L + (-1)* abs(FunCoef(xL))/2 * (-p_L)' * (-p_L),...% xL
            ( p_R)' * FunCoef(xR) * p_R/2 + FluxVal * abs(FunCoef(xR))/2 * ( p_R)' *   p_R,...
            ( p_R)' * FunCoef(xR) * p_L/2 + FluxVal * abs(FunCoef(xR))/2 * ( p_R)' * (-p_L),...% xR
            ];
...
%             ];
    TraVal = [...
        (-p_L)' * FunCoef(xL) * p_R/2 + FluxVal * abs(FunCoef(xL))/2 * (-p_L)' *   p_R,...
        (-p_L)' * FunCoef(xL) * p_L/2 + FluxVal * abs(FunCoef(xL))/2 * (-p_L)' * (-p_L),...% xL
        ( p_R)' * FunCoef(xR) * p_R,...( p_R)' * FunCoef(xR) * p_R/2 + (1) * abs(FunCoef(xR))/2 * ( p_R)' *   p_R,...
        ( p_R)' * (p_R-p_R),...( p_R)' * FunCoef(xR) * p_L/2 + (1) * abs(FunCoef(xR))/2 * ( p_R)' * (-p_L),...% xR
        ];
    end
