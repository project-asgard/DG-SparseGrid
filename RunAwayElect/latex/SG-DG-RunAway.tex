...
\subsection{Example 1} In this test, we choose a proper source term such that the exact solution is 
...
\subsection{Example 2} In this test, we choose a proper source term such that the exact solution is 
...
\end{eqnarray}
The DG method will be applied to approximate the above equation.
...
\end{eqnarray*}
with $f_0=\exp(-x^2/\sigma^2),\ \sigma=0.1.$
...
\begin{tabular}{c}
  ...
  \end{tabular}
\caption{Test 1: Plot for solutions for Lev = 4, T = 0.5.}
\end{figure}
...
Let
\begin{eqnarray*}
\frac{\partial f}{\partial t} =  \frac{\partial}{\partial x}(1-x^2)f + \frac{\partial}{\partial x}(1-x^2)\frac{\partial f}{\partial x}.
\end{eqnarray*}
First, we shall 
