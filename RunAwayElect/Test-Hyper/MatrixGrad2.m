function Mat = MatrixGrad2(Lev,Deg,LInt,LEnd,FluxVal,FunCoef,FunCoef2,bcL,bcR)
...
%     end
    ...
    if WorkCel == 0
        Iu = RowInd;%(:,Deg+1:end);
        Iv = [ones(Deg,1)*(c-Deg)+Deg*(Tol_Cel_Num ),ones(Deg,1)*c,ones(Deg,1)*c,ones(Deg,1)*(c+Deg)];%ColInd(:,Deg+1:end);
        Val = TraVal;%(:,Deg+1:end);
    elseif WorkCel == Tol_Cel_Num - 1
        Iu = RowInd;%(:,1:3*Deg);
        Iv = [ones(Deg,1)*(c-Deg),ones(Deg,1)*c,ones(Deg,1)*c,ones(Deg,1)*[1:Deg]];
    %ColInd(:,1:3*Deg);
        Val = TraVal;%(:,1:3*Deg);
    else
        Iu = RowInd;
