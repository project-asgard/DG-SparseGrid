...
\begin{eqnarray}
\begin{cases}
(q,v) &= \left(\dfrac{\partial f}{\partial x},v\right),\\
\left(x^2\dfrac{\partial f}{\partial t},w\right) &= \left(\dfrac{\partial}{\partial x}\left[x\psi(x)q\right],w\right)+\left(\dfrac{\partial}{\partial x}\left[2x^2\psi(x)f\right],w\right)\\
& = -\left(x\psi(x)q,\dfrac{\partial w}{\partial x}\right)+\langle \widehat{x\psi(x)q},w\rangle\\
&\quad -\left(2x^2\psi(x)f,\dfrac{\partial w}{\partial x}\right)+\langle \widehat{2x^2\psi(x)f},w\rangle
\end{cases}
\end{eqnarray}
Thus
\begin{eqnarray}
...
Here $\textbf{Mat}_1$ is the Grad operator with FunCoef = 1. The matrix $\textbf{Mat}_2$ is the Grad operator with FunCoef = $x\psi(x)$.
