...
% Major Changes:
% 1. Hash Table only contains Lev and Cel, but ignoring Deg
...
for i=1:HASH.dof
    ...
    Index = Deg^Dim*(i-1)+1:Deg^Dim*i; % correct one
...
A_encode=GlobalMatrixSG(vMassV,GradX,HASH,HASHInv,Con2D,Deg);
...
tmp=Multi_2D(Meval_v,Meval_x,fval,HASH,Inv,Deg);
...
for L = 1:floor(TEND/dt)
    ...
    B_encode = GlobalMatrixSG(GradV,EMassX,HASH,HASHInv,Con2D,Deg);
