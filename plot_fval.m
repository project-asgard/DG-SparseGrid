...
%     dof2=deg2*2^lev2;
    ...
    stat = mkdir('output_dt_0,05');
    fName = ['output_dt_0,05/f2d-' sprintf('deg_%d_lev_%d',deg1, lev1) '.mat'];
    save(fName, 'x', 'y', 'f2d');
    %%
   ...
   % end
    ...
    fNamex = ['output_dt_0,05/f1d_x-' sprintf('deg_%d_lev_%d',deg1, lev1) '.mat'];
    save(fNamex, 'x', 'f1d');
 ...
 %   semilogy(x_FG_coarse, f1d_FG_coarse, '-g', 'LineWidth', 2);
  ...
  %  semilogy(x_FG_deg7, f1d_FG_deg7, '-ob');
   ...
   % hold off;
    ...
    x = nodes{1};
    y = nodes{2};
    fNamey = ['output_dt_0,05/f1d_y-' sprintf('deg_%d_lev_%d',deg1, lev1) '.mat'];
    save(fNamey, 'y', 'f1d');
