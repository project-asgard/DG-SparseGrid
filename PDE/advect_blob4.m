function pde = advect_blob4(opts)
% 4D advect a blob across a domain. Use for feature tracking testing in the adaptiviy.  
%
% df/dt == -v * df/dx
%
% Run with
%
% asgard(@advect_blob4,'deg',5','lev',4,'num_steps',5,'dt',0.002)
%
% Notes:
...
dim_y = DIMENSION(-1,+1);
dim_z = DIMENSION(-1,+1);
dim_vx = DIMENSION(-1,+1);
dimensions = {dim_x,dim_y,dim_z,dim_vx};
...
a_vx = @(x,p,t) exp(-x.^2/p.sig.^2);
a_t = @(t,p) t.*0+1;
sol1 = new_md_func(num_dims,{a_x,a_y,a_z,a_vx,a_t});
solutions = {sol1};
