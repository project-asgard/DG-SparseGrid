...
function [err,fval,fval_realspace] = asgard_ItterIIF (pde, varargin)
...
for L = 1:num_steps
    ...
    while needs_adapting
        ...
        fval_unstepped = fval;
        fval = time_advance_itterIIF(pde,opts,A_data,fval,t,dt,pde.deg,hash_table,[],[]);
        fval_realspace = wavelet_to_realspace(pde,opts,Meval,fval,hash_table);
        ...
        if opts.adapt
            ...
            if num_elements_0 == num_elements_adapted
                ...
                fval = time_advance_itterIIF(pde,opts,A_data,fval_unstepped_adapted,t,dt,pde.deg,hash_table,[],[]);
                fval_realspace = wavelet_to_realspace(pde,opts,Meval,fval,hash_table);
            else
                if ~opts.quiet
