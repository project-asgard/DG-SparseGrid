function f = time_advance_itterIIF(pde,opts,A_data,f,t,dt,deg,hash_table,Vmax,Emax)
%-------------------------------------------------
...
%         b = 2*f0 + dt*(ALHS \ A)*f0 + dt*(ALHS \ (s0+s1+bc0+bc1));
        ...
        [~,A] = apply_A_1(pde,opts,A_data,f0,deg);
        B = A;
        [~,C] = apply_A_2(pde,opts,A_data,f0,deg);
        I = eye(numel(diag(A)));
        AA = 2*I - dt*A;
    ...
    save('matrix1','B') %save matrix1
    save('matrix2','C') %save matrix2
